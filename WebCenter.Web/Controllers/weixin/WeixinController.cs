...
namespace WebCenter.Web.Controllers
{
    public class WeixinController : BaseController
    {
        ...
        public WeixinController(IUnitOfWork UOF)
            : base(UOF)
        {
        }     
        ...
        public ActionResult Post(string signature, string timestamp, string nonce, string echostr)
        {
            ...
            using (var streamReader = new StreamReader(Request.InputStream))
            {
                ...
                if (safeMode)
                {
                    ...
                    if (ret != 0)//解密失败
                    {
                       ...
                       Common.LogHelper.LogError(string.Format("decrypt message return {0}, request body {1}", ret, msg));
                    }
                }
                else
                {
            ...
            if (safeMode)
            {
                ...
                {
                    //TODO：开发者加密失败的业务处理逻辑
                    Common.LogHelper.LogError(string.Format("encrypt message return {0}, response body {1}", ret, response));
                }
            }
            else
            {
